...
function DashboardPriorityFeedRow({ item, index }: DashboardPriorityFeedRowProps) {
  const content = (
    <>
      <span
        aria-hidden="true"
        className="flex size-8 shrink-0 items-center justify-center rounded-full bg-muted font-semibold text-muted-foreground text-sm tabular-nums"
      >
        {index + 1}
      </span>
  ...
  if (item.href === undefined) {
    return (
      <div className="grid grid-cols-[auto_minmax(0,1fr)] items-start gap-3 rounded-lg border border-transparent p-3 text-card-foreground sm:p-4">
        {content}
      </div>
    );
  }
  ...
  return (
    <InertiaActionLink
      ...
      className="grid min-h-11 grid-cols-[auto_minmax(0,1fr)_auto] items-start gap-3 rounded-lg border border-transparent p-3 text-card-foreground transition-[background-color,border-color,transform] duration-motion-fast ease-standard hover:border-border hover:bg-muted/60 active:translate-y-px focus-visible:outline-2 focus-visible:outline-offset-2 focus-visible:outline-ring motion-reduce:transition-none motion-reduce:active:translate-y-0 sm:p-4"
    >
      {content}
      ...
      <section aria-labelledby="dashboard-priority-feed-heading">
        <header className="border-border border-b px-5 py-4 sm:px-6">
          ...
          <p className="mt-1 text-muted-foreground text-sm">
            重要ニュース、シグナル、分析待ちから種類ごとに最大3件を表示しています。
          </p>
        </header>
            ...
            <ol className="flex flex-col gap-2 p-3 sm:p-4">
              {items.map((item, index) => (
                <li key={`${item.kind}-${item.id}`}>
                  <DashboardPriorityFeedRow item={item} index={index} />
                </li>
              ))}
            </ol>

            <footer className="flex justify-end border-border border-t px-5 py-3 sm:px-6">
              <InertiaActionLink
                href={newsIndex.url()}
                pendingClassName="opacity-70"
