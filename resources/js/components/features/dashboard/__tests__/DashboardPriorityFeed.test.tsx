...
describe('DashboardPriorityFeed', () => {
  it('確認候補を受け取った順序で表示すること', () => {
    ...
    expect(
      screen.getByText('重要ニュース、シグナル、分析待ちから種類ごとに最大3件を表示しています。'),
    ).toBeVisible();
    expect(renderedItems).toHaveLength(2);
    expect(renderedItems.map((item) => within(item).getByText(/^[12]$/).textContent)).toEqual([
      '1',
      '2',
    ]);
  ...
  it('遷移先がない候補はリンクにせず表示すること', () => {
    ...
    // Act
    const title = screen.getByText('3件のニュースが分析待ちです');

    // Assert
    expect(title).toBeVisible();
    expect(
      screen.queryByRole('link', { name: /3件のニュースが分析待ちです/ }),
  ...
  it('項目が空の場合は正常な空状態と銘柄一覧への導線を表示すること', () => {
    ...
    // Assert
    expect(screen.getByText('現時点で優先して確認する項目はありません')).toBeVisible();
