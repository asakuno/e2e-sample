...
4. **ルール連携**: `.Codex/rules/security/` と連携した包括的なガイダンス

**重要**: このスキルは `.Codex/rules/security/` のルールと**協調動作**する。rulesが「守るべき規約」を提供し、本スキルは「実行する診断ワークフロー」を提供する。
...
このスキルは `.Codex/rules/security/` のルールと以下のように連携する:
